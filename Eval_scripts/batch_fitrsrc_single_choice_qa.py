...
import re
def extract_choice_label(prediction):
    # The regular expression pattern for choice label
    pattern = r'\b([A-E])\b'
    # Find all matches in the prediction
    matches = re.findall(pattern, prediction)
    # If we found any matches, return the first one. Otherwise, return None.
    return matches[0] if matches else None

def evaluation_metrics(data_path):
    base = [json.loads(q) for q in open(data_path, "r")]
    category_correct = {"relationship": 0, "object": 0, "subject": 0, "exist": 0}
    category_incorrect = {"relationship": 0, "object": 0, "subject": 0, "exist": 0}
    # A structure to store answers grouped by question_id and category
    grouped_answers = defaultdict(lambda: defaultdict(list))
    for answer in base:
        question_id = answer['question_id']
        category = answer['category'].lower()
        gt = answer['ground_truth'].lower()
        ans = answer['answer'].lower()
        if gt == ans:
            grouped_answers[question_id][category].append(True)
        else:
            if extract_choice_label(answer['answer']):
                choice_label = extract_choice_label(answer['answer']).lower()
                grouped_answers[question_id][category].append(gt == choice_label)
            else:
                grouped_answers[question_id][category].append(False)
                
    correct = 0
    incorrect = 0
    for question_id, categories in grouped_answers.items():
